...
class PassServer < Sinatra::Base
  ...
  post '/test' do
    ...
    p = { :random => values["random"], :name => values["name"] }
    userid = add_user_with_params(p)
    "{\"success\": #{userid}}"
  end
